...
REPORT_PATH = os.path.join(_DIR, "reports", "fraud_model_advanced_report.md")
...
def plot_class_conditional_densities(conditionals, feature_names, save_dir):
    ...
    fig.suptitle("Class-Conditional Distributions — P(Feature | Class)", fontsize=13, fontweight="bold", y=1.02)
    fig.tight_layout()
    ...
    for idx, trace in enumerate(update_traces):
        ...
        for bar, prob in zip(bars, probs):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.02,
                    f"{prob:.3f}", ha="center", fontsize=7)
...
def plot_calibration_curves(fitted_models, X_test, y_test, save_dir):
    ...
    ax.set_title("Calibration Curves — Predicted vs. Actual Probability", fontsize=13, fontweight="bold")
...
def plot_causal_dag(dag, save_dir):
    ...
    nx.draw_networkx_labels(dag, pos, labels=labels, ax=ax, font_size=8, font_weight="bold")

    ax.set_title("Hypothesised Causal DAG for Insurance Fraud", fontsize=14, fontweight="bold", pad=20)
    ax.axis("off")
    fig.tight_layout()
...
def plot_propensity_scores(ps_results, save_dir):
    ...
    ax1.set_title("Propensity Score Distributions", fontsize=12, fontweight="bold")
    ...
    ax2.set_xlabel("Propensity Score Stratum (Quintile)", fontsize=11)
    ax2.set_ylabel("Treatment Effect (Fraud Rate Diff)", fontsize=11)
    ax2.set_title("Stratum-Level Treatment Effects", fontsize=12, fontweight="bold")
...
def plot_importance_comparison(comparison_df, save_dir):
    ...
    ax1.set_yticklabels([f.replace("_", " ").title() for f in top_perm["feature"]], fontsize=8)
    ax1.invert_yaxis()
    ax1.set_xlabel("Permutation Importance (F1 drop)", fontsize=10)
    ...
    if len(kl_data) > 0:
        ...
        ax2.set_yticklabels([f.replace("_", " ").title() for f in kl_data["feature"]], fontsize=8)
        ax2.invert_yaxis()
    ax2.set_xlabel("KL Divergence (P(x|fraud) || P(x|legit))", fontsize=10)
    ax2.set_title("Bayesian Likelihood Divergence\n(Class-Conditional KDE)", fontsize=11, fontweight="bold")
